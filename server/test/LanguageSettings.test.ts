...
describe('Validate LanguageSettings', () => {
    it('tests merging language settings', () => {
        ...
        expect((sPython.dictionaries || []).sort()).to.be.deep.equal(['wordsEn', 'companies', 'softwareTerms', 'node', 'python', 'misc'].sort());
        ...
        expect((sPhp.dictionaries || []).sort())
            .to.be.deep.equal(['wordsEn', 'companies', 'softwareTerms', 'node', 'php', 'html', 'fonts', 'css', 'typescript', 'misc'].sort());
    });
});